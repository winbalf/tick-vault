        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                r = await client.get(
                    url,
                    params={
                        "ids": ",".join(settings.coingecko_ids),
                        "vs_currencies": "usd",
                    },
                )
                r.raise_for_status()
            ...
            if not isinstance(raw, dict):
                raise ValueError(f"unexpected coingecko payload type={type(raw)}")

            event_ts_ms = int(time.time() * 1000)
            for coin in settings.coingecko_ids:
                inner = raw.get(coin) or raw.get(coin.lower()) or {}
                usd = inner.get("usd") if isinstance(inner, dict) else None
                if usd is None:
                    raise ValueError(f"missing usd for id={coin!r} keys={list(raw.keys())}")
